...
def sample_from_discretized_mix_logistic(y, log_scale_min=-7.0, clamp_log_scale=False, greedy=False):
    """
...
def mixture_loss(y_hat, y, log_scale_min=-7.0, reduce=True, dist_cls=Normal):
    ...
    centered_y = y - means
    dist = dist_cls(loc=0., scale=torch.exp(log_scales))
...
def sample_from_mixture(y, log_scale_min=-7.0, greedy=False, dist_cls=Normal):
    """
    ...
    if greedy:
        ...
        dist = dist_cls(loc=means, scale=scales)
        x = dist.sample()
